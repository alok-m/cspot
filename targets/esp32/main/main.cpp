...
// #endif
    ...
    ESP_LOGI(TAG, "AAAAAAAAAAAAAAAA6----1");    
    ESP_LOGI(TAG, "1: %d, 2: %d", sizeof(FileHelper), sizeof(ESPFile));
    ESP_LOGI(TAG, "AAAAAAAAAAAAAAAA6----1.5");    
    std::shared_ptr<FileHelper> otherFile;
    otherFile = std::make_shared<ESPFile>();
    std::shared_ptr<FileHelper> test(otherFile);
    ESP_LOGI(TAG, "AAAAAAAAAAAAAAAA6----2");    

    configMan = std::make_shared<ConfigJSON>("/spiffs/config.json", otherFile);
    ESP_LOGI(TAG, "AAAAAAAAAAAAAAAA7----");

    bell::JSONObject obj;
    obj["username"] = "test";

    otherFile->writeFile("/spiffs/test.json", obj.toString());
